...
latest = session_data.get_latest_bar("AAPL")  # ~0.05µs
```
...
bars = session_data.get_last_n_bars("AAPL", 20)  # ~1.2µs
```
...
```python
# Get latest bar (O(1))
session_data.get_latest_bar(symbol, interval=1)

# Get last N bars (O(n))
session_data.get_last_n_bars(symbol, n=20, interval=1)

# Get bars since timestamp (O(k) where k = bars after timestamp)
session_data.get_bars_since(symbol, timestamp, interval=1)

# Get bar count (O(1))
session_data.get_bar_count(symbol, interval=1)

# Batch get latest for multiple symbols (O(n))
session_data.get_latest_bars_multi(["AAPL", "GOOGL", "MSFT"])
```
...
latest_bars = session_data.get_latest_bars_multi(
    ["AAPL", "GOOGL", "MSFT", "TSLA"]
)

# INEFFICIENT: Multiple calls
for symbol in symbols:
    bar = session_data.get_latest_bar(symbol)  # Don't do this!
```
...
recent = session_data.get_bars_since("AAPL", five_min_ago)
...
latest = session_data.get_latest_bar("AAPL")
last_20 = session_data.get_last_n_bars("AAPL", 20)
...
latest_all = session_data.get_latest_bars_multi(symbols)
```
...
for i in range(100):  # BAD
    bar = session_data.get_latest_bar("AAPL")
```
